...
export const DiabloHUD: React.FC<DiabloHUDProps> = ({
  ...
  return (
    <div id="diablo-hud-container" className="pointer-events-none absolute inset-0 flex flex-col justify-between p-4 select-none">
      {/* Top Bar */}
      <TopStatusBar
        player={player}
        boss={boss}
      ...
      <div className="flex items-end justify-center gap-2 md:gap-4 relative">
        ...
        <div className="pointer-events-auto relative z-20 flex flex-col items-center -mr-3">
          <div className="relative h-28 w-28 md:h-32 md:w-32 rounded-full border-4 border-stone-800 bg-stone-950 p-1 shadow-[0_0_25px_rgba(220,38,38,0.5)] overflow-hidden">
            <div
          ...
          <div className="mt-1 flex gap-1">
            <button
              ...
              className="rounded border border-stone-700 bg-stone-900/90 px-2 py-1 text-[11px] font-bold text-amber-400 shadow hover:bg-stone-800 active:scale-95 transition-all flex items-center gap-1"
            ...
            <button
              onClick={onOpenCamp}
              className="rounded border border-amber-600/80 bg-amber-950/80 px-2 py-1 text-[11px] font-bold text-amber-300 shadow hover:bg-amber-900 active:scale-95 transition-all"
        ...
        <div className="pointer-events-auto relative z-10 flex flex-col items-center rounded-t-xl border-t-2 border-x-2 border-stone-700 bg-gradient-to-b from-stone-900/95 to-stone-950/95 px-4 pt-2 pb-1 shadow-2xl backdrop-blur-md">
          ...
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <div className="group relative flex flex-col items-center">
              <div
                className="relative flex h-11 w-11 items-center justify-center rounded border-2 border-amber-500/80 bg-stone-800 text-stone-100 shadow-inner"
                style={{ boxShadow: `0 0 8px ${dynStats.vfx.glowColor}55` }}
              >
                <span className="text-xl">{primaryAttackInfo.icon}</span>
                ...
                <span className="absolute bottom-0.5 right-1 text-[9px] font-bold text-amber-400">L-CLK</span>
              </div>
            ...
            <div className="group relative flex flex-col items-center">
              <div className="relative flex h-11 w-11 items-center justify-center rounded border-2 border-stone-600 bg-stone-800 text-stone-100 shadow-inner">
                <span className="text-xl">{secondaryAttackInfo.icon}</span>
                <span className="absolute bottom-0.5 right-1 text-[9px] font-bold text-amber-400">R-CLK</span>
              </div>
            ...
            <div className="group relative flex flex-col items-center">
              <div className="relative flex h-11 w-11 items-center justify-center rounded border-2 border-stone-600 bg-stone-800 text-stone-100 shadow-inner">
                ...
                <span className="absolute bottom-0.5 right-1 text-[9px] font-bold text-amber-400">空格</span>
              </div>
              <span className="mt-0.5 text-[9px] text-stone-400">战术冲刺</span>
            </div>

            <div className="h-8 w-px bg-stone-700 mx-0.5" />
            ...
            {['1', '2', '3', '4', 'q'].map((hotkey) => {
              ...
              return (
                ...
                >
                  <div className={`relative flex h-11 w-11 items-center justify-center rounded border-2 ${skill ? 'border-amber-500 bg-amber-950/80 hover:border-amber-400' : 'border-stone-600 bg-stone-800 hover:border-stone-500'} text-white shadow-inner`}>
                    {skill ? <span className="text-xl drop-shadow">{skill.icon}</span> : defaultIcon}
                    {cooldown > 0 && (
                      <div className="absolute inset-0 bg-black/75 flex items-center justify-center text-xs font-mono font-bold text-cyan-400">
                        {cooldown.toFixed(1)}
                      </div>
                    )}
                    <span className="absolute bottom-0.5 right-1 text-[9px] font-bold text-amber-400">{hotkey.toUpperCase()}</span>
                  </div>
                  <span className="mt-0.5 text-[9px] text-stone-400">{skill ? skill.name.split(' ')[0] : defaultName}</span>
                </button>
              );
            })}
          </div>
          ...
          <div className="relative w-full flex flex-col items-center">
            ...
            <div className="flex justify-between w-full text-[9px] font-mono text-stone-400 px-1 mt-0.5">
              ...
              <span>WASD移动 · 左键攻击 · 右键特技 · 空格冲刺</span>
            </div>
          </div>
        </div>

        {/* Right: Giant Blue Mana/Energy Globe */}
        <div className="pointer-events-auto relative z-20 flex flex-col items-center -ml-3">
          <div className="relative h-28 w-28 md:h-32 md:w-32 rounded-full border-4 border-stone-800 bg-stone-950 p-1 shadow-[0_0_25px_rgba(37,99,235,0.5)] overflow-hidden">
            <div
